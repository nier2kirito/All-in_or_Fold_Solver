...
from parser_2 import aggregate_hand_data_from_files
import random
import csv
...
TOP_20_PERCENT_HANDS = [
    # Pocket pairs (55+)
    "5 5o", "6 6o", "7 7o", "8 8o", "9 9o", "10 10o", "J Jo", "Q Qo", "K Ko", "A Ao",
    # Suited aces (A9s+)
    "9 As", "10 As", "J As", "Q As", "K As",
    # Offsuit aces (ATo+)
    "10 Ao", "J Ao", "Q Ao", "K Ao",
    # Additional suited broadways
    "10 Ks", "J Ks", "10 Qs", "10 Js",
    # Offsuit broadway
    "Q Ko"
]
...
hand_data_old = aggregate_hand_data_from_files(old = True)
...
def naive_player_decisions(hand):
    ...
    if hand in  TOP_20_PERCENT_HANDS:
        return 'ALL_IN'
    return 'FOLD'
...
def simulate_game(num_games=1, strategic_player_index=2, player= naive_player_decisions,runs =1, rake = 0.03, jackpot_fee = 0.035, jackpot_payout = 1.5, threshold_value = 9, starting_stacks = [8,8,7.5,7], evaluator=None):
    ...
    for _ in range(num_games):
        ...
        # Initialize jackpot_eligible variable
                ...
                for (i,card) in alive_hands:
                    if i in winners:
                        ...
                        if min_val <= threshold_value:
                            ...
                            wins[i] += (sum([starting_stacks[j] for (j,hand) in alive_hands]) + 1.4 - rake + jackpot_payout - jackpot_fee) / len(winners) - starting_stacks[i]
                        else:
                            wins[i] += (sum([starting_stacks[j] for (j,hand) in alive_hands]) + 1.4 - rake) / len(winners) - starting_stacks[i]
                    else:
                        wins[i] -= starting_stacks[i]   # Assign negative values for losers
        
        win_rates = [wins[i] / runs for i in range(4)]    
        try:
            assert (sum(win_rates) < 1e-5 ) or (sum(win_rates) + 0.03) < 1e-5 or (jackpot_eligible and(sum(win_rates) +0.03 - jackpot_payout + jackpot_fee) < 1e-5 )#whether a pot was constructed or not
        except AssertionError:
            print('Alive hands:', alive_hands)
            print("Decisions:", decisions)
            print("Wins:", wins)
            print("Sum Wins:", sum(wins))
...
def save_to_csv_general(num_games = 1, player= naive_player_decisions):
    ...
    for _ in range(num_games // 4):
        for index in range(4):
            results+=simulate_game(num_games=1, strategic_player_index = index, player = naive_player_decisions,evaluator=evaluator)
            for i, (hands, decisions, win_rates) in enumerate(results):
                writer.writerow(hands + decisions + win_rates)
# Function to calculate exploitability

def simulate_game_new(num_games=1, strategic_player_index=2,runs =1, rake = 0.03, jackpot_fee = 0.035, jackpot_payout = 1.5, threshold_value = 9, starting_stacks = [8,8,7.5,7], evaluator=None):
    results = []
    
    for game_number in range(num_games):
        temp_results=[]
        start_time = time.time()  # Start time for ETA calculation
        # Here many cards we should decide if they can be dealt twice or not 
        hands = random.sample(hands_combinations, 4)
        decisions = []
        
        # Initialize jackpot eligibility
        jackpot_eligible = False  # Initialize jackpot_eligible here
        ...
        for player_index in [2,3,0,1]:
            if player_index == strategic_player_index:
                ...
                if decisions == ['FOLD', 'FOLD', 'FOLD']:
                    ...
                    infoset = decision_to_infoset(decisions)
                    fold_prob, all_in_prob = hand_data_old[(infoset, parse_hand(hands[mapping[player_index]]))]
                    if random.random() < all_in_prob:
                        decisions.append('ALL_IN')
                    else:
                        decisions.append('FOLD')
        ...
        win_rates = [wins[i] / runs for i in range(4)]    
        try:
            assert (sum(win_rates) < 1e-3 ) or (sum(win_rates) + 0.03) < 1e-5 or (jackpot_eligible and(sum(win_rates) +0.03 - jackpot_payout + jackpot_fee) < 1e-5 )#whether a pot was constructed or not
        except AssertionError:
            print('Alive hands:', alive_hands)
            print("Decisions:", decisions)
            print("Wins:", win_rates)
        temp_results+=[parse_hand(hand) for hand in hands]
        temp_results+=decisions
        temp_results+=win_rates
        results.append(temp_results)
        # ETA calculation
        elapsed_time = time.time() - start_time
        remaining_games = num_games - (game_number + 1)
        estimated_time = elapsed_time * remaining_games
        
        # Convert estimated time to minutes and seconds
        minutes, seconds = divmod(estimated_time, 60)
        print(f"ETA: {int(minutes)} minutes, {int(seconds)} seconds remaining", end='\r')  # Print ETA in the same line
    print()  # Move to the next line after completion
    return results

# Function to save results to CSV
def save_to_csv_new(strategic_player_index=2, num_games = 1):
    evaluator = Evaluator()
    results = simulate_game_new(num_games, strategic_player_index, evaluator=evaluator)
    filename = f"game_results_{strategic_player_index}.csv"
...
import time
_NUM_SIMULATIONS = 10**7
if __name__ == "__main__":
    aggregated_data = aggregate_hand_data_from_files()
    start_time = time.time()
    player = naive_player_decisions
    save_to_csv_new(0, _NUM_SIMULATIONS // 4)
    save_to_csv_new(1, _NUM_SIMULATIONS // 4)
    save_to_csv_new(2, _NUM_SIMULATIONS // 4)
    save_to_csv_new(3, _NUM_SIMULATIONS // 4)
    # save_to_csv_general(1000)
