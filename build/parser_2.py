    ...
    with open(file_path, 'r') as file:
        ...
        for line in lines:
            ...
            if "InfoSet" in line:
                ...
                # Extract visits if available
                visits = 1  # Default value if visits not found
                visits_index = line.find("Visits:")
                if visits_index != -1:
                    visits_part = line[visits_index:].strip()
                    try:
                        visits = int(visits_part.split(':')[1].strip())
            ...
            hand_data[(decisions,card_section)] = (fold_prob, all_in_prob, visits)
...
def aggregate_hand_data(hand_datas):
    ...
    # Initialize hand_data_final with zeros
    for hand_data in hand_datas:
        for (decisions, hand), (fold_prob, all_in_prob, visits) in hand_data.items():
            if (decisions, hand) not in hand_data_final:
                hand_data_final[(decisions, hand)] = (0, 0)
                visits_data[(decisions, hand)] = 0

    # Aggregate data from all hand_data entries with visit weights
    for hand_data in hand_datas:
        for (decisions, hand), (fold_prob, all_in_prob, visits) in hand_data.items():
            current_fold_prob, current_all_in_prob = hand_data_final[(decisions, hand)]
            current_visits = visits_data[(decisions, hand)]
            
            # Add weighted probabilities
            hand_data_final[(decisions, hand)] = (
                current_fold_prob + fold_prob * visits,
                current_all_in_prob + all_in_prob * visits
            )
            visits_data[(decisions, hand)] = current_visits + visits

    # Calculate weighted average
    for key in hand_data_final:
        fold_prob_sum, all_in_prob_sum = hand_data_final[key]
        total_visits = visits_data[key]
        
        if total_visits > 0:
            hand_data_final[key] = (
                fold_prob_sum / total_visits,
                all_in_prob_sum / total_visits
            )
        else:
            # If no visits, keep as zeros
            hand_data_final[key] = (0, 0)
...
def aggregate_hand_data_from_files(old = False):
    if not old:
        file_paths =glob.glob('corrected_final_infosets_*.txt')
    else:
        file_paths =glob.glob('infosets_updated_2_blidns_0_4_*.txt')
    hand_data_all = []
...
#     # Save aggregated results to a pickle file
#     with open('aggregated_results.pkl', 'wb') as f:
#         pickle.dump(hand_data_final, f)
