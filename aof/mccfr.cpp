...
public:
    ...
    int visitCount;  // Add visit count field
};
...
// Node implementation
Node::Node() : regretSum(3, 0.0), strategy(3, 0.0), strategySum(3, 0.0), visitCount(0) {}
...
void saveInfoSetsToFile(const std::string& filename) {
    ...
    for (const auto& entry : nodeMap) {
        file << "InfoSet: " << entry.first << " Visits:" << entry.second.visitCount << "\n";
        std::vector<double> avgStrat = entry.second.getAverageStrategy();
        file << "Strategy: ";
        for (double s : avgStrat) {
